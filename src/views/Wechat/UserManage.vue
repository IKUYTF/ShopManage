<template>
    <div class="data-table">
        <v-toolbar flat color="white">
            ...
            <v-text-field label="検索..." append-icon="search" v-model="word" @input="searchWord"></v-text-field>
            <v-spacer></v-spacer>
            <v-spacer></v-spacer>
            <v-spacer></v-spacer>
            <v-dialog v-model="dialog" max-width="450px">
                <template v-slot:activator="{ on }">
                    <v-btn color="#1867c0" dark class="mb-2" v-on="on">ユーザー追加</v-btn>
                </template>
                <v-card>
                    <v-card-title>
                                ...
                                <v-layout wrap>
                                    <v-flex xs12 md12>
                                        <v-text-field v-model="editedItem.username" :counter="20"
                                                      ...
                                                      label="ユーザー名" required :disabled="editedIndex > -1"
                                    ...
                                    <v-flex xs12 md6>
                                        <v-btn :disabled="!valid" color="success" class="btn" @click="save">確認
                                        </v-btn>
                                    </v-flex>
...
<script>
    ...
    export default {
        ...
        created() {
            this.localUrl = global.backServerEndpoint + '/user';
            const user = localStorage.getItem('ms_userAuth');
            if(user === 'false') {
        ...
        methods: {
            ...
            deleteItem(item) {
                ...
                const url = this.localUrl + '/shop-delete/' + item.username;
                this.$axios({
                    method: 'get',
                    url: url
                }).then((res) => {
                    this.snackbar = true;
                    this.alertText = 'ユーザー情報削除済み';
                    this.initialize();
                });
